package com.uwu.study.db.mq.rocket.component;
...
public class CustomMessageListenerConcurrently implements MessageListenerConcurrently {
    ...
    public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext consumeConcurrentlyContext) {
        ...
        consumeConcurrentlyContext.setDelayLevelWhenNextConsume(1); //表示重试间隔为1s
        MessageExt msg = list.get(0);
        log.info("received msg：{}", msg);
        try {
            log.info("Receive New Messages：{}", list);
            ...
            System.out.println("------------------最大重试次数为:" + list.get(0).getReconsumeTimes() + "次!--------------------");
            System.out.println("-------延迟级别设置:" + consumeConcurrentlyContext.getDelayLevelWhenNextConsume());
            long d = System.currentTimeMillis();
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            System.out.println("当前时间:" + sdf.format(d));
            if (list.get(0).getReconsumeTimes() > 3) {
                consumeConcurrentlyContext.setDelayLevelWhenNextConsume(-1); //重试大于3次直接发往死信队列
            }
